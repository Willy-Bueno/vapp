...
function presentialInterview(id: string) {
  ...
  if (peopleId) {
    const promise = () =>
      new Promise((resolve) =>
        resolve(
          responseStore.createResponse({ people_id: peopleId, survey_id: id }).then((data) =>
            router.push({
...
<template>
  <div>
    <p class="text-sm text-muted-foreground">Aqui aparece os questionários que estão <strong>publicados</strong> no momento.</p>
    <p class="text-sm text-muted-foreground">Selecione um questionário para começar a entrevista.</p>
  </div>
      ...
      <Sheet>
        ...
        <SheetContent class="min-w-fit w-full">
          <SheetHeader>
            ...
            <SheetDescription> Escolha a opção de entrevista que você deseja realizar. </SheetDescription>
          </SheetHeader>
