<script setup lang="ts">
  ...
  questions: Array<Tables<'questions'> &
    { options?: Array<Tables<'options'>> } &
  ...
  const responseSchema = z.record(z.object({
  ...
  function handleSubmit () {
    ...
    const promise = () => new Promise((resolve) => resolve(answerStore.createAnswer(new AnswerData(response.value, response_id).result)
    .then(() => {
  ...
  onMounted(async () => {
    ...
    survey.value?.questions.forEach((question) => {
      response.value[question.id] = {
        ...
        answer_multiple_options: question.question_types.slug === 'multiple' &&
          question.options &&
...
<template>
  ...
  <div v-else v-for="question in survey?.questions" class="max-w-2xl w-full flex flex-col gap-2">
    ...
    <div v-else-if="question.question_types.slug === 'multiple'">
      <div class="flex flex-col gap-2">
        <div v-for="option in question.options" :key="option.id" class="flex items-center justify-start space-x-2">
          <Checkbox type="" :id="option.id" :value="option.id" :checked="(response[question.id]?.answer_multiple_options?.[option.id])" @update:checked="(checked: boolean) => {
            return checked ?
              response[question.id].answer_multiple_options[option.id] = true :
              response[question.id].answer_multiple_options[option.id] = false
            }" />
    ...
    <div v-else-if="question.question_types.slug === 'unique'">
      <RadioGroup
        ...
        v-model="response[question.id].answer_unique_option"
      >
