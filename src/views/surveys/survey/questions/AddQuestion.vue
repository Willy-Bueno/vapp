...
function handleSubmit() {
  ...
  const promise = () =>
    new Promise((resolve) =>
      resolve(
        questionStore.getQuestions(surveyId).then(() => {
    ...
    <form @submit.prevent="handleSubmit">
      ...
      <CardContent class="grid grid-cols-2 gap-2 pt-6 justify-center w-full space-y-8">
        <div class="grid gap-2 col-span-2 w-full">
          <Label for="name" :class="cn('text-sm', errors?.question_text && 'text-destructive')"> Título da questão </Label>
          <Input id="name" v-model="questionForm.question_text" placeholder="ex: Qual é o seu provedor de internet atualmente?" />
          <span class="text-muted-foreground text-sm"> Certifique-se de que a pergunta seja clara e objetiva. </span>
        ...
        <div class="grid gap-2 col-span-2 md:col-span-1">
          <Label for="question_type" :class="cn('text-sm', errors?.question_type && 'text-destructive')"> Tipo de questão </Label>
          <Popover>
            ...
            <PopoverContent class="p-0" align="end">
              <Command v-model="questionForm.question_type.title" class="flex flex-col w-full">
                <CommandList>
                  <CommandEmpty> Nenhum tipo de questão encontrado. </CommandEmpty>
                  <CommandGroup>
                    <CommandItem
          ...
          <span class="text-muted-foreground text-sm"> Selecione o tipo de questão que você deseja adicionar. </span>
        ...
        <div class="grid gap-2 col-span-2 md:col-span-1 content-start">
          <Label for="required_question" :class="cn('text-sm', errors?.required && 'text-destructive')"> Essa questão é obrigatória? </Label>
          ...
          <span class="text-muted-foreground text-sm"> Selecione se essa questão pode ser pulada ou não. </span>
        ...
        <div v-if="questionForm.question_type?.slug === 'text'" class="grid gap-2 col-span-2 w-full">
          <Label for="question-type-text-placeholder" class="text-sm"> Placeholder do campo de texto <span class="text-muted-foreground">(Opcional)</span> </Label>
          <Input name="question-type-text-placeholder" v-model="questionForm.placeholder" placeholder="ex: Qual seu provedor de internet?" />
          <span class="text-muted-foreground text-sm"> O placeholder ajuda o usuário a entender o que deve ser preenchido no campo de texto. </span>
        ...
        <div v-if="['multiple', 'unique'].includes(questionForm.question_type.slug)" class="grid gap-2 col-span-2 lg:col-span-1 w-full">
          <Label :class="cn('text-sm', errors?.options && 'text-destructive')"> Opções </Label>
          <draggable
              ...
              <div>
                <div class="flex items-center space-x-2">
                  ...
                  <Button
                    ...
                    @click="
                      questionForm.options?.splice(
                        questionForm.options?.findIndex((option) => option.key === element.key),
                        1
                      )
                    "
                  >
                  ...
                  <Button type="button" variant="outline" :disabled="questionForm.options?.length === 1" class="flex items-center handle">
                ...
                <div v-if="errors?.options?.[questionForm.options?.findIndex((option) => option.key === element.key)]?.option_text" class="text-sm text-destructive">
                  <template>
                    <div v-for="error in errors.options[questionForm.options?.findIndex((option) => option.key === element.key)].option_text?._errors" :key="error">
                      <p>{{ error }}</p>
                    </div>
                  </template>
                </div>
          ...
          <div class="flex justify-start">
            <Button type="button" variant="outline" @click="addOption"> Adicionar opção </Button>
          </div>
          <span class="text-muted-foreground text-sm"> As opções são as alternativas que o usuário pode escolher. </span>
        </div>
      </CardContent>
      <Separator />
      <CardContent class="grid grid-cols-2 gap-2 pt-6 justify-center w-full space-y-8">
        <div class="grid gap-2 col-span-2 w-full pointer-events-none">
          <Label for="name" class="text-sm"> Pré-visualização </Label>
          ...
          <CardContent class="grid grid-cols-2 gap-2 pt-6 justify-center w-full space-y-2">
            <div v-if="!questionForm.question_text || !questionForm.question_type.slug" class="p-12 grid col-span-2 justify-center items-center">
              <p class="text-muted-foreground text-center">Preencha o título e o tipo da questão para pré-visualizar.</p>
            </div>
            <div v-else class="grid col-span-2 space-y-2">
              ...
              <div v-if="questionForm.question_type.slug === 'text'" class="grid gap-2 col-span-2 w-full">
                <Input v-model="questionForm.placeholder" :placeholder="questionForm.placeholder" />
              </div>
              <div v-if="questionForm.question_type.slug === 'multiple'" class="grid gap-3 col-span-2 w-full">
                <div v-for="option in questionForm.options" :key="option.option_text" class="flex items-center space-x-2">
                  ...
                  <label :for="option.option_text" class="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
                    {{ option.option_text }}
                  </label>
                </div>
              </div>
