...
const statusOptions = computed(() => surveyStatusStore.surveyStatus?.map(status => ({
  value: status.id,
  label: status.title,
})) || [])
...
<template>
  ...
  <form v-if="!isLoading" class="space-y-8">
    ...
    <div class="grid gap-2">
      <Label for="status" :class="cn('text-sm', errors?.survey_status_id && 'text-destructive')">
        Status
      </Label>
      ...
      <span class="text-muted-foreground text-sm">
        Esse é o status do seu questionário.
      </span>
      <span class="text-muted-foreground text-sm">
        Obs: Não é possivel publicar um questionário que não tenha nenhuma questão.
      </span>
