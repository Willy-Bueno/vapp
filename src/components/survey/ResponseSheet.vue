  ...
  <Sheet>
    ...
    <SheetContent class="min-w-full w-full min-h-fit overflow-y-scroll">
      ...
      <div class="flex flex-1 flex-col items-center px-4 md:px-8 py-36 space-y-16 lg:mt-32 lg:ml-96">
        <div v-for="question in props.response.surveys.questions" :key="question.id" class="max-w-2xl w-full flex flex-col gap-2">
            ...
            <div class="flex flex-col gap-2">
              <div v-for="option in question.options" :key="option.id" class="flex items-center justify-start space-x-2 pointer-events-none">
                <Checkbox
                  ...
                  :checked="
                    option.id ===
                    question.answers.find((answer) => answer.response_id === props.response.id)?.answer_options?.find((answerOption) => answerOption.question_option_id)?.question_option_id
                  "
                />
